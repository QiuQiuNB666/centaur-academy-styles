# 半人马AI学院 · 刊本（C / Edition） — Style Reference

> 黑场开幕，半人马随滚动放大，撕开进入奶油纸页；章节名用喊的，故事用讲的，标签用记的。

**Theme:** 开幕 dark（纯黑 #000000）→ 正文 light（橄榄奶油 #f1efe6）→ 收尾 dark（橄榄深墨 #1b1b13）

这一版的骨架照 Shopify Editions Winter '26：先是一块只有黄金分割构图线、细白画框和标题锁定的黑场，框里站着一只小小的半人马；往下滚，它在钉住的舞台里从 0.46 倍放大到 1 倍，构图线同步向外展开，放完之后，页面沿一条像素撕纸边切进奶油纸页。正文是「三声部」编排：Noto Sans SC 900 的巨型紧排字喊章节名，Noto Serif SC 讲叙述句，Geist Mono 小字记编号、日期、图注。颜色只有两个——橄榄奶油和橄榄墨——加一组同色相的橄榄灰；logo 里的橘橙 #f07800 是唯一的界面强色，只出现在顶栏色条、主 CTA、开幕标题的强调词、第一步瓦片的大数字和引号上，全页面积不超过 3%；logo 的钴蓝一滴都不进界面，留给 logo 和插画。插画不做滤镜，像画作一样放进衬纸（#e4e1d4）里装裱，外加 1px 墨线内框和一行等宽图注。所有面零圆角、零阴影，只有按钮和标签是胶囊；层次靠发丝线和底色切换。签名记忆点是开幕的**「画框放大」**：黑场里的细画框先把半人马装成一幅小画，滚动时画和框一起长大，画框标签（PLATE I）与图注沿对角线被推到放大后的框角。
...
| 橄榄墨           | `#25251b` | `--color-ink`                | 正文文字、分区头黑线、第一步瓦片、院长寄语左半、焦点环       |
| 橄榄深墨         | `#1b1b13` | `--color-ink-deep`           | 联系区与页脚的暗底                                           |
| 发丝             | `#d2cfc0` | `--color-hair`               | 纸面分隔线、描边瓦片、小瓦片大数字                           |
| 暗面发丝         | `#3a3a2e` | `--color-hair-dark`          | 暗面分隔线、顶栏竖线                                         |
...
| 暗面次要         | `#a9a898` | `--color-on-dark-muted`      | 黑场小字；黑底 8.7:1、深墨底 7.2:1                           |
...
| 纸面构图线       | `rgba(37,37,27,.07)` | `--color-construct-paper` | 正文里贯穿全页的三根竖向发丝；按钮图标底               |
...
- **Role:** 标题里的「AI」、步骤大数字 01–05。替代 Shopify 的 NeueMontreal，比 Inter 更窄更硬。

### Noto Serif SC — 讲：叙述句、章节 H2、引文、图注 · `--font-serif-zh`

- **Substitute:** "Songti SC", STSong, SimSun
- **Weights:** 500, 700
- **Sizes:** 15px, 17px, 20px, 26px, 30px, 46px, 50px
...
- **Role:** ② 的 Geist Mono 署名与标签。只在 11–14px 出现，从不放大。
...
| display   | clamp(44px, 5.4vw + 8px, 92px)（开幕 H1 放大到 clamp(44px, 6.4vw + 8px, 104px)） | 1.06        | -0.02em        | `--text-display`   |
...
| body-sm   | 15px                               | 1.7         | 0              | `--text-body-sm`   |
| caption   | 13px                               | 1.5         | 0              | `--text-caption`   |
| label     | 12px mono 大写                     | 1.4         | +0.06em        | `--text-label`     |

规律照 ②④：字越大字距越紧、行高越贴；但中文字距封底 -0.02em、展示级行高不低于 1.02。

## Tokens — Spacing & Shapes

**Base unit:** 4px（`--spacing: 4px`）

**Density:** 疏——章节之间 200px 上下，区块内部 40–90px，章节标题上下留白每章不同
...
两条曲线都取自 Shopify Editions 的实测值。**滑动放大**：`.stage` 声明 `view-timeline: --stage block`，舞台内层 `position: sticky`，额外滚动距离 `min(88vh, 760px)`（<120vh）；`.figure-box` 从 `scale(.46)` 到 `scale(1)`，构图线 SVG 从 `scale(1) / opacity 1` 到 `scale(1.28) / opacity .6`，画框标签与图注沿对角线平移 28% 的框半径；`animation-range: contain 0% contain 92%`，曲线 ease-settle。整段写在 `@supports (animation-timeline: view())` + `min-width:1024px` + `prefers-reduced-motion: no-preference` 里，`animation-timeline` 写在 `animation` 简写之后。默认样式就是终态，所以不支持的浏览器、减少动效、1023px 以下看到的都是放大后的静态画面。图框按终态排版（`min(760px, 52vw)`），只从小放到 1，2 倍屏不糊。只动 transform 和 opacity。

### Layout

- **Section gap:** 章节 176–216px，区块内 40–90px
...
**Role:** 全站唯一的固定层
高 56px，纯黑底。左起：248px 宽字标块（logo 26px + 「半人马AI学院」900/17px + 等宽 CENTAUR AI ACADEMY），右侧 1px 暗面发丝；紧接满高橘橙色条 `clamp(200px,24vw,340px)`，文字「联系学院顾问 ↗」贴右；再往右是纯文字导航 15px/700，hover 变橘橙。1023px 以下导航收进「菜单 +」橘橙块，展开为黑底全宽列表，「+」旋转 45° 成「×」。

### Primary Button（accent 胶囊）

**Role:** 主 CTA「联系学院顾问」
`bg --color-accent` `color --color-accent-foreground`，`border-radius: var(--radius-pill)`，高 48px，`padding: 4px 4px 4px 22px`，15px/700。右端嵌 40px 墨色圆，圆里是橘橙 ↗（按钮套按钮）。hover：底色 `--color-accent-press`，图标圆 `translate(2px,-2px)`；active：`scale(.97)`；focus：2px 环 + 3px 偏移。每屏最多一个。
...
**Role:** 「下载知君 ↓」「下载万象 ↓」
`disabled` 属性，衬纸底 + 发丝边 + 橄榄灰字，图标圆透明、不响应 hover；右侧并排等宽小字「下载暂未开放」。

### 分区头（Section Rule）

**Role:** ② 的「线 + 等宽标签」
1px 墨色通栏线，线下 12px：左端等宽标签（01 / 、CENTAUR JOURNAL / 观点与动态、FROM THE FOUNDER），右端 Instrument Serif 罗马数字。每个区块都以它开头。

### 章首喊字（Shout）

**Role:** 章节名
Noto Sans SC 900，148px，行高 1.02，字距 -0.02em，一行到底。章 I 左齐、章 II 右齐、章 III 左齐且上方留白最短——三章三种位置，不重复。

### 画作式装裱框（Plate）

**Role:** 三张章节插画的唯一处理
`figure.plate` → 衬纸 `.mat`（clamp 18–64px 内边距）→ 插画 1448:1086 + 1px 墨线内框 → 图注行：左等宽英文（EXPERIENCE）、右衬线中文（｜ 经验，值得新的可能。），下边 1px 发丝。插画不调色、不加滤镜。

### 期刊卡（Journal Card）

**Role:** 观点与动态
6 + 3 + 3 列错落（第二、三张分别下沉 120 / 240px）。封面衬纸 12–24px 内边距，4:3 裁切；学院主视觉那张用 `mix-blend-mode: multiply` 把烤在图里的米色底融进衬纸。下方：胶囊标签「观点」+ 等宽日期 → 21px/700 标题 → 15px 橄榄灰摘要。hover 封面放大 1.02。

### 共学五步瓦片（Steps Tiles）

**Role:** 五步真实顺序
12 列、10px 间距的不等宽瓦片：01 占 5 列 × 2 行（墨底、橘橙巨型「01」贴右下），02 占 4、03 占 3、04 占 3、05 占 4；02/05 衬纸底，03/04 透明底 + 发丝边。每块：左上等宽 STEP、右上 76px 发丝色数字、底部 900 步骤名 + 衬线说明。

### 产品下载行（Product Row）

**Role:** 知君 / 万象
Shopify 章末清单式：顶部 1px 墨线，每行下边发丝。左 56px/900 产品名，右上衬线 20px 链接「— 让积累参与思考 ↗」，右下禁用下载按钮 + 等宽说明。
...
**Role:** 类型标记
胶囊，高 26px，1px 墨线，等宽 11px 大写。变体：accent（橘橙实底）、muted（发丝边 + 灰字）。
...
| 3     | ink         | `#25251b` | 第一步瓦片、引文左半                      |
| 4     | ink-deep    | `#1b1b13` | 联系区、页脚                              |

层级之间不用阴影，靠硬切：黑 → 奶油用像素撕纸边，奶油 → 深墨用反向撕纸边，纸内的层级只差一个衬纸色。
...
- 橘橙 `#f07800` 全页累计面积 ≤3%，只放在：顶栏色条、主 CTA、开幕强调词、01 大数字、院长引号。
- 章节名用 Noto Sans SC 900 喊，叙述用 Noto Serif SC 讲，编号日期用 Geist Mono 记——三个声部不串岗。
- 强调词换声部（衬线 → 900 黑体），不换颜色。开幕 H1 是唯一例外（强调词用橘橙）。
- 插画一律进衬纸装裱，带 1px 墨线内框和等宽图注。
...
两类图，两种处理，各自贯穿全站。**抠图半人马**只出现在黑场里：透明底版站在细白画框中，背后是黄金分割构图线（两个相切大圆 + 两条 30° 对角线 + 两组嵌套黄金矩形），像一幅被量过比例的古典画稿；滚动时画和框一起长大。**三张暖色场景插画**只出现在纸页上：不调色，放进衬纸，墨线内框，下方一行「等宽英文 ｜ 衬线中文」图注——这是 Shopify「画作式大留白」在学院素材上的落地，暖插画配橄榄衬纸，色温差被衬纸吸收。期刊卡复用同三张图，学院主视觉（米色底烤在图里）用 multiply 融进衬纸。图形只用内联 SVG：构图线、像素撕纸边。没有图标库，箭头用等宽字符 ↗ ↓ →。

## Layout

页面是三段：56px 黑色书脊顶栏；钉住的黑场舞台（四角 + 中心构图：左上眉标与两行 104px 标题，右上衬线导语与按钮，中心画框与半人马，左下衬线主张，右下等宽 INDEX）；奶油纸页正文。正文照 Shopify 做成「左 248px 固定目录 + 右 12 列内容」：目录贴在视口下部，七个条目带点线引到罗马数字；内容区背后有三根贯穿全页的纸面发丝竖线（Dropbox 的网格延续开幕构图线）。三章节依次是「文窄图宽（4 + 7 列，底对齐）」「图大文窄（8 + 4 列，文字下沉）」「文 + 清单（5 + 6 列）再接 9 列大图」；共学五步是不等宽瓦片；观点三卡 6 + 3 + 3 逐级下沉；院长寄语 5 : 7 对开；收尾是反撕纸边后的居中暗场与满宽字标。1199px 以下目录收起；1023px 以下舞台不再钉住，改为「眉标 → 标题 → 画框 → 导语 → 主张」的静态堆叠，所有栅格落成单列；600px 以下瓦片单列、按钮满宽。
...
2. Create the primary CTA: pill, `#f07800` background, `#25251b` 15px/700 text, 48px tall, padding 4px 4px 4px 22px, a 40px `#25251b` circle flush right containing an orange "↗". Hover `#d86a00` and nudge the circle `translate(2px,-2px)` with `cubic-bezier(.72,.16,.19,.96)`; active `scale(.97)`.
3. Create the pinned opening: black stage, full-bleed SVG golden-ratio construction lines at `rgba(241,239,230,.17)` and a 380×520 frame at `.42`; a transparent cutout centered, laid out at its final width `min(760px,52vw)` and scroll-scaled from `.46` to `1` via `view-timeline` inside `@supports (animation-timeline: view())`; construction SVG scales `1 → 1.28` while fading to `.6`. Default styles equal the end state.
...
- **④ 发丝网格即装饰**（③）— 开幕构图线网格 + 正文三根贯穿竖向发丝。出处：`references/dropbox-brand/fold.png`、`study/dropbox-brand/s07.png`、`s08.png`（内页网格线贯穿留白）。
...
- **⑦ 黑场开幕 → 纸页正文**（④）— 构图线 + 细框 + 标题锁定，像素撕纸边切入。出处：`references/shopify-editions-winter26/fold.png`（黄金分割构图线）、`study/shopify-editions-winter26/s01.png`（细白框）、`s04.png` / `s12.png`（撕纸边）。
...
- **⑩ 图像只有一种处理**（④）— 画作式衬纸装裱 + 等宽图注，全站三张插画同一手法。出处：`study/shopify-editions-winter26/s07.png`、`s09.png`（画作当卡底、大留白）。
- **⑪ 不对称首屏**（②）— 标题在左、导语在右上、主视觉居中，四角各放一件事。出处：`references/ai-in-design-report/fold.png`（左导语、右大标题）。
- **⑫ 章节标题上下留白每章不同**（④）— 136/112、216/128、96/176px，喊字左/右/左轮换，图文组合不重复。出处：`study/shopify-editions-winter26/PATTERNS.md` 节奏总结与 `s03.png` / `s13.png`（两章章首构图不同）。
- 另借：② 的引文对开（`study/ai-in-design-report/s05.png`）→ 院长寄语；② 的黑底居中订阅 + 满宽字标页脚（`s12.png`、`s13.png`）→ 联系区与页脚；④ 的章末两栏清单（`study/shopify-editions-winter26/s12.png`）→ 知君 / 万象下载行；③ 的错落卡组（`study/dropbox-brand/s10.png`）→ 观点三卡错落。
...
- 跳转链接：`跳到正文`
- 复制按钮反馈：`已复制`
- 组件样张页（kit.html）的说明文字与区块名：`组件样张 · C / EDITION`、`刊本：黑场开幕，纸页正文。`、`色板`、`三声部`、`胶囊按钮 · 全状态`、`画作式装裱 · 构图线 · 撕纸边`、`期刊卡 · 徽标`、`共学五步瓦片`、`引文对开 · 产品下载行`、`01 / COLOR` 等分区头、输入框示例 `你的问题` / `聊聊你正在做的事` / `暂未开放`
