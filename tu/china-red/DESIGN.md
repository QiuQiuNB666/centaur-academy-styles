...
「中国红大会」把官网当成一场企业家年会来布置：开幕是一整块朱砂红舞台（#a30f17 → #6e0910 径向渐变），中心一枚米白奖章托起半人马，12 道细金光芒只占 0.8° 宽、透明度 30%，比年会 PPT 的放射光克制得多。标题用粗宋 900 撑住分量，整页只有「你的能力。」一处用马善政毛笔题字并做金箔渐变，右上角压一方「半人马AI学院」朱文方印（手机上改盖在题字右下角内侧，离屏幕边 ≥16px）。金色是细线资源：边框 1px、回纹 10px 高、牌匾外框 1px，题字、牌匾字和主按钮之外不出现大块金。正文区一律回到宣纸米白 #fbf5ea，墨色正文 #2a1a14 对比 15.4:1，次要字 #5a4638 也有 8.17:1，适合 50 多岁的老板在微信里看。版式居中对称：共学五步做成典礼流程单，院长寄语做成上下带轴头的题词卷轴，联系区像报名台（红底上一张米白台面、顶边一条金）。签名记忆点是**奖章登场**：滚动时米白奖章从 0.5 放到 1，金色光芒同时展开并回正 24°，奖章背后一圈细金刻度盘（3° 细刻度 + 30° 主刻度 + 两道同心圆 + 十字经纬线）从暗转亮并回正 40°，一道斜向扫光（overlay 混合，在金边和半人马上显出金属反光）掠过奖章，最后亮出托在绛红名牌条上的「HUMAN × AI」图注。
...
**奖章登场（签名动效）：** 外层 `.stage-zoom` 用 `view-timeline`，里面的奖章 `position:sticky` 钉在视口中线附近；区间从奖章顶边进入视口底部开始，到额外滚动 `min(36svh,300px)` 为止。奖章 `scale(.5→1)`，光芒 `scale(.6→1) rotate(-24°→0)`、透明度 .25→1，扫光在进度 40% 后从左掠到右（伪元素不能放进 `:is()`，`.sweep::before` 的 `animation-timeline` 必须单独写一条选择器，否则扫光走文档时间线、0 秒就结束，永远看不到），刻度盘 `.dial`（奖章外扩 13%）同一时间线上 `rotate(-40°→0)`、透明度 .12→1；光芒和刻度盘同装在不旋转的 `.halo`（奖章外扩 35%）里，`.halo` 用 `mask-image:linear-gradient(transparent 20%,black 40%)` 让上半圈在奖章顶边以上完全消失，保证金线不从按钮和「了解共学与共创 →」文字底下穿过（实测 390/768/1440 各滚动位置，遮罩起点始终在链接底边之下）；桌面 `.stage-figure` 上内边距 28px，静态终态时奖章顶边离主按钮底边 28px；图注在最后 35% 淡入；图注托在绛红底 + 上下 1px 金线的名牌条上，避免 16px 字落在光心红上（直接压光心只有 4.86:1）。写在 `@supports (animation-timeline: view())` 里，`animation-timeline` 放在 `animation` 简写之后；不支持时（安卓微信、iOS 26 以下）JS 兜底：IntersectionObserver 只在舞台附近挂 passive scroll，rAF 里只写 transform/opacity；`?nosd` 强制走兜底；`prefers-reduced-motion` 和视口高于 1600px 时直接显示终态。实测半人马终态宽度：390 宽 372px、768 宽 666px、1440 宽 659px，均在分辨率天花板内。
...
绛红底 + 1px 赤金边 + 5px 外偏移的半透明金线，字为粗宋 900 金箔渐变，28–44px。`word-break:keep-all`，窄屏只在逗号处折行（390 宽「每一次共学，/ 都向前一点」），不会剩单字。
...
**Role:** 品牌落款
米白底朱文，竖排两列「半人马 / AI学院」（AI 横排合字），双线内框。开幕跟在题字右上角，全站只盖一次；手机（<600px）改为绝对定位在题字右下角内侧（`top:100%; right:0`，不占行高），右缘离屏幕边 ≥16px，不再贴边。
...
- 科技质感只留两处：奖章背后的细金刻度盘（随登场由暗转亮，终态静止）；红底牌匾标题进入视口时一次性金色扫光（`mix-blend-mode:screen`，1.6s，只播一次，IntersectionObserver 触发，reduced-motion 下不触发）。
- 首屏要看得到主角：开幕上下间距收紧（手机顶距 20px、桌面 24px，舞台不再加上边距，奖章缩放原点 50% 10%），390×844 与 1440×900 下 scrollY=0 时奖章和半人马上半身都在首屏内，主按钮也在首屏内。
...
- 不要让扫光、光芒、刻度盘循环播放——登场随滚动走一次，牌匾扫光进视口只播一次；不要彩色光效。
- 不要用纯黑 #000 或 Inter / Roboto / Arial。

## Imagery

主视觉只有一个物件：透明底半人马放在米白奖章里，奖章外是绛红圈和细金光芒，思路和华为全联接大会「一个 3D 物件当主视觉」一致，但换成颁奖盛典的奖章语言。三张暖色插画不上红底，一律进白衬纸画框，靠红角花把它们拉进红金体系。期刊封面直接用插画与 `academy-centaur-hero.webp`，裁 16:10。装饰全部是 CSS / 内联 SVG：网格（6% 金）、回纹、光芒、刻度盘、印章，没有位图装饰。
...
1. 做一块开幕：朱砂径向渐变底 + 6% 金网格，居中粗宋 900「让 AI 成为」，下一行马善政题字金箔渐变「你的能力。」，右上角米白朱文方印（手机放题字右下角内侧）；奖章背后加一圈细金刻度盘随登场由暗转亮；下面 56px 金箔按钮「联系学院顾问 ↗」。
