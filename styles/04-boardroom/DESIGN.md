...
**Role:** 首屏主视觉的底
内联 SVG：外圈 + 72 刻度（`stroke-dasharray`）+ 十字经纬 + 虚线内圈，全部 `primary-border`；一个 214 半径的黄铜圆和一根指向右上的黄铜指针（1.2px）带实心箭头。抠图半人马叠在上面，指针从躯干后穿出、顺着视线方向。全站只出现一次。
